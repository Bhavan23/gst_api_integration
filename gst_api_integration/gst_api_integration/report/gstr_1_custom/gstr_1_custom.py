			...
			for inv, items_based_on_rate in self.items_based_on_tax_rate.items():
				...
				for rate, items in items_based_on_rate.items():
					...
					if self.filters.get("type_of_business") in ("CDNR-REG", "CDNR-UNREG"):
						row.append("Y" if invoice_details.posting_date <=
						           date(2017, 7, 1) else "N")
	...
	def get_row_data_for_invoice(self, invoice, invoice_details, tax_rate, items):
		...
		for fieldname in self.invoice_fields:
			if (
				...
				row.append(abs(invoice_details.base_rounded_total)
				...
				export_type = "WPAY" if invoice_details.get(
					fieldname) == "With Payment of Tax" else "WOPAY"
			...
			if d.item_tax_rate:
				...
				for account, rate in item_tax_rate.items():
					tax_rate_dict = self.item_tax_rate.setdefault(
						d.parent, {}).setdefault(d.item_code, [])
	...
	def get_columns(self):
		...
		if self.filters.get("type_of_business") == "B2B":
			self.invoice_columns = [
				...
				{"fieldname": "customer_name", "label": "Receiver Name",
				...
				{"fieldname": "posting_date", "label": "Invoice date",
			...
			self.other_columns = [
				{"fieldname": "cess_amount", "label": "Cess Amount",
			...
			self.invoice_columns = [
				...
				{"fieldname": "reverse_charge", "label": "Reverse Charge", "fieldtype": "Data"},
				{"fieldname": "export_type", "label": "Export Type",
			...
			self.other_columns = [
				...
				{"fieldname": "document_type", "label": "Document Type",
					"fieldtype": "Data", "width": 80},
			]
		elif self.filters.get("type_of_business") == "CDNR-UNREG":
			self.invoice_columns = [
				...
				{"fieldname": "posting_date", "label": "Note Date",
			...
			self.invoice_columns = [
				...
				{"fieldname": "port_code", "label": "Port Code",
					"fieldtype": "Data", "width": 120},
				{
			...
			self.invoice_columns = [
				{"fieldname": "place_of_supply", "label": "Place Of Supply",
					"fieldtype": "Data", "width": 120}
			]
			...
			self.invoice_columns = [
				{"fieldname": "description", "label": "Description",
					"fieldtype": "Data", "width": 420},
				{"fieldname": "nil_rated", "label": "Nil Rated",
					"fieldtype": "Currency", "width": 200},
				{"fieldname": "exempted", "label": "Exempted",
					"fieldtype": "Currency", "width": 200},
				{"fieldname": "non_gst", "label": "Non GST",
					"fieldtype": "Currency", "width": 200},
			]

		self.columns = self.invoice_columns + self.tax_columns + self.other_columns
		
		# self.columns += [{"fieldname": "is_uploaded",
		#                     "label": "Is Uploaded", "fieldtype": "Check", "width": 30}]
...
def get_json(filters, report_name, data):
	...
	gstin = get_company_gstin_number(filters.get(
		"company"), filters.get("company_address"))

	fp = "%02d%s" % (getdate(filters["to_date"]).month,
	...
	if filters["type_of_business"] == "B2B":
		...
		for item in report_data[:-1]:
			res.setdefault(item["export_type"], {}).setdefault(
				item["invoice_number"], []).append(item)
...
def get_advances_json(data, gstin):
	...
	for place_of_supply, items in iteritems(data):
		supply_type = "INTRA" if company_state_number == place_of_supply.split(
			"-")[0] else "INTER"
		row = {"pos": place_of_supply.split(
			"-")[0], "itms": [], "sply_ty": supply_type}
...
def download_json_file():
	...
	frappe.response["filename"] = (
		frappe.scrub("{0} {1}".format(
			data["report_name"], data["report_type"])) + ".json"
	)
